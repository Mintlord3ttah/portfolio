...
export default function Navbar() {
  ...
  const diplay_list = menu ? "max-[700px]:h-fit max-[700px]:gap-4 max-[700px]:p-6 max-[700px]:overflow-hidden" :
  ...
  return (
    <nav onClick={handleRemoveMenu} className={`nav-${theme} transition-all duration-300 flex z-50 fixed top-0 left-0 w-full justify-between items-center py-6 px-24 max-[1200px]:px-8 max-[500px]:px-4 shadow-sm`}>
        ...
        <Logo />
        <button onClick={()=>setMenu(prev=>!prev)} className='btn hidden max-[700px]:block text-2xl'>{menu ? <FaTimes /> :<IoMenuOutline />}</button>
        </div>
      <ul className={`${diplay_list} nav-menu-${theme} flex space-x-8 transition-all duration-500 font-bold text-gray-500 max-[700px]:flex-col max-[700px]:absolute max-[700px]:top-full max-[700px]:left-0 max-[700px]:w-full`}>
        <List label="Projects" />
        <List label="Skills" />
        <List label="About" />
        <List label="Contact" />
        <li onClick={handleTheme} className='max-[700px]:w-fit max-[700px]:px-2.5 cursor-default max-[700px]:mt-6 max-[620px]:bg-amber-700 max-[620px]:text-white font-light gap-1.5 flex items-center justify-center hover:bg-[#c46b31] hover:text-white transition-all duration-300 size-6 rounded-sm '>
